...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        print('start Allocated:', round(start_mem/(1024**2), 1), 'MB')
        x = self.conv2(x)
        end_mem = torch.cuda.memory_allocated(0)
        print('stop Allocated:', round(end_mem/(1024**2), 1), 'MB')
        print(f'diff Allocated:{round((end_mem - start_mem)/(1024**2), 1)} MB')
        #ncu.stop()
        #quit()
        x = F.relu(x)
        x = F.max_pool2d(x, 2)
